...
import {
  fetchDmMessages, fetchChannelMessages, ackDmMessages, ackChannelMessages,
  getDevicePublicInfo,
} from '../api';
import { decryptFromDevice, dhPublicKeyB64, uuidToBytes } from '../crypto';
import type { DecryptedMessage, InboundMessage } from '../types';
...
type Props = DmProps | ChannelProps;
...
export default function MessageList(props: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (ackIds.length) {
        ...
        if (props.kind === 'dm') ackDmMessages(session.token, props.id, ackBody).catch(() => {});
        else ackChannelMessages(session.token, props.id, ackBody).catch(() => {});
      }
      ...
      if (last) setCursor(last.batch_id);
    })();
  ...
  return (
    <div className="flex flex-1 flex-col-reverse overflow-y-auto scrollbar-thin px-4 py-2">
      <div ref={bottomRef} />
      {messages.length === 0 && (
      ...
      {[...messages].reverse().map(msg => (
        <MessageBubble
          key={msg.batchId}
          msg={msg}
          isSelf={msg.senderUserId === session.userId}
        />
      ))}
    </div>
  );
...
function MessageBubble({ msg, isSelf }: { msg: DecryptedMessage; isSelf: boolean }) {
  const time = msg.createdAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  return (
    <div className={`mb-1 flex items-start gap-2 ${isSelf ? 'flex-row-reverse' : ''}`}>
      <div className="mt-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-surface-500 text-xs font-bold uppercase">
      ...
      <div className={`max-w-[75%] ${isSelf ? 'items-end' : 'items-start'} flex flex-col`}>
        <div className={`flex items-baseline gap-2 ${isSelf ? 'flex-row-reverse' : ''}`}>
          <span className="text-xs font-semibold text-gray-300">{msg.senderUsername}</span>
        ...
        <div
          className={`mt-0.5 rounded-2xl px-3 py-1.5 text-sm ${
            isSelf
              ...
              : 'rounded-tl-sm bg-surface-600 text-gray-100'
          }`}
        >
          {msg.plaintext}
        </div>
...
function errMessage(raw: InboundMessage, reason: string): DecryptedMessage {
  return {
