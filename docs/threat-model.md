...
- **Channel history for new members (policy)**: Whether **intentional**
history handoff is **permitted** is controlled by a boolean set by the **server
owner only** (`allow_past_channel_history_for_new_members`; see
`docs/protocol.md` 10.1). Default should be **off**. When on, **any current
server member** whose device received the relevant ciphertext may act as
**donor** (decrypt locally, re-encrypt to the new member’s devices); the owner
does **not** have to be the donor. Relay and product gates only — no
server-side plaintext; donors re-encrypt on their own devices when the flag
allows.
